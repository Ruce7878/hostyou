...
echo "<wml>\n";
if(!isset($rm) || $rm==""){
if ($row["level"]>6)echo "<card id=\"Ok\" title=\"Operater Ban\" ontimer=\"enter.php?$ses&amp;ref=$ref\"><timer value=\"10\" />\n";
else echo "<card id=\"Ok\" title=\"Operater Ban\" ontimer=\"enter.php?$ses&amp;ref=$ref\"><timer value=\"10\" />\n";
}else{
echo "<card id=\"Ok\" title=\"Operater Ban\" ontimer=\"chat.php?$ses&amp;rm=$rm&amp;ref=$ref\"><timer value=\"10\" />\n";
}
echo "<p align=\"center\">\n";
}else{
echo "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">";
...
echo $fsize1;
$browsers = mysql_fetch_array(mysql_query ("SELECT user, level, user_ip, user_soft, hostname FROM users WHERE id='".$nk."'"));
if($browsers[2]==$browsers[4]){
$brwsssss = explode(".",$browsers[4]);
$brwsssss1 = explode(" ",$browsers[3]);
$result=count($brwsssss);
$hostban="$brwsssss[0].$brwsssss[1]";
}else{
$brwsssss = explode(".",$browsers[4]);
$brwsssss1 = explode(" ",$browsers[3]);
$result=count($brwsssss);
$result1=$result-1;
$result2=$result-2;
$hostban="$brwsssss[$result2].$brwsssss[$result1]";
}
if($browsers[1]<$row["level"]){
echo "<b>$browsers[0]</b> je banovan(Operater)!<br/>\n";
$bnik = check($bnik);
mysql_query ("INSERT INTO bannlist SET ip = '".$browsers[2]."', soft = '".$brwsssss1[0]."', user = '".$browsers[0]."', hostname = '".$hostban."', tip='4', time='".time()."'");
$room="room".$rm;
...
$time = time();
$room = "room".$rm;
$txt = "".$us." je banovao(Operater) <b>".$browsers[0]."</b>, zbog krsenja pravila!";
...
flock ($open,LOCK_EX);
fwrite($open,"$browsers[0]-$browsers[2]-$brwsssss,$data|who: $us|$REMOTE_ADDR|$HTTP_USER_AGENT\n");
fflush($open);
flock ($open,LOCK_UN);
fclose($open);
/////////////////////////////////////////////////////////////////////
...
$text="$levelnamesa[0] <b>$namenski</b> je banovao(Operater) <b>$browsers[0]</b> zbog krsenja pravila!<br/>Operater: $hostban";
