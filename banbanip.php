...
echo $fsize1;
echo "<b>Nemate prava pristupa!!!Mislili ste ako nikog nema da mozete raditi sta hocete? E pa NE MOZE :).</b><br/>\n";
echo $fsize2;
if ($ver=="wml")echo "</p></card></wml>";
else echo "</div></body></html>";
mysql_close ($link);
...
echo "<wml>\n";
if(!isset($rm) || $rm==""){
if ($row["level"]>6)echo "<card id=\"Ok\" title=\"IP Ban\" ontimer=\"enter.php?$ses&amp;ref=$ref\"><timer value=\"10\" />\n";
else echo "<card id=\"Ok\" title=\"Browser Ban\" ontimer=\"enter.php?$ses&amp;ref=$ref\"><timer value=\"10\" />\n";
}else{
echo "<card id=\"Ok\" title=\"IP Ban\" ontimer=\"chat.php?$ses&amp;rm=$rm&amp;ref=$ref\"><timer value=\"10\" />\n";
}
echo "<p align=\"center\">\n";
}else{
echo "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">";
...
echo "<b>$browsers[0]</b> je banovan(IP)!<br/>\n";
$bnik = check($bnik);
mysql_query ("INSERT INTO bannlist SET ip = '".$browsers[2]."', soft = '".$browsers[3]."', user = '".$browsers[0]."', tip='2', time='".time()."'");
$room="room".$rm;
...
$time = time();
$room = "room".$rm;
$txt = "".$us." je banovao(IP) <b>".$browsers[0]."</b>, zbog krsenja pravila!";
...
flock ($open,LOCK_EX);
fwrite($open,"$browsers[0]-$browsers[2]-$browsers[3],$data|who: $us|$REMOTE_ADDR|$HTTP_USER_AGENT\n");
fflush($open);
flock ($open,LOCK_UN);
fclose($open);
/////////////////////////////////////////////////////////////////////
...
$text="$levelnamesa[0] <b>$namenski</b> je banovao(IP) <b>$browsers[0]</b> zbog krsenja pravila!<br/>IP:".$_SERVER["REMOTE_ADDR"]." ,Browser:".$_SERVER["HTTP_USER_AGENT"]."";
