...
echo "<wml>\n";
if(!isset($rm) || $rm==""){
if ($row["level"]>6)echo "<card id=\"Ok\" title=\"Browser Ban\" ontimer=\"enter.php?$ses&amp;ref=$ref\"><timer value=\"10\" />\n";
else echo "<card id=\"Ok\" title=\"Browser Ban\" ontimer=\"enter.php?$ses&amp;ref=$ref\"><timer value=\"10\" />\n";
}else{
echo "<card id=\"Ok\" title=\"Browser Ban\" ontimer=\"chat.php?$ses&amp;rm=$rm&amp;ref=$ref\"><timer value=\"10\" />\n";
}
echo "<p align=\"center\">\n";
}else{
echo "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">";
...
$brwsssss = explode(" ",$browsers[3]);
if($browsers[1]<$row["level"]){
echo "<b>$browsers[0]</b> je banovan(Browser+)!<br/>\n";
$bnik = check($bnik);
mysql_query ("INSERT INTO bannlist SET ip = '".$browsers[2]."', soft = '".$brwsssss[0]."', user = '".$browsers[0]."', tip='3', time='".time()."'");
$room="room".$rm;
...
$time = time();
$room = "room".$rm;
$txt = "".$us." je banovao(Browser+) <b>".$browsers[0]."</b>, zbog krsenja pravila!";
...
$text="$levelnamesa[0] <b>$namenski</b> je banovao(Browser+) <b>$browsers[0]</b> zbog krsenja pravila!<br/>IP:".$_SERVER["REMOTE_ADDR"]." ,Browser:".$_SERVER["HTTP_USER_AGENT"]."";
