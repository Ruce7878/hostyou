...
$mat_text[]= 'Внимание! Продается очко блядины '.$us.' сильно б.у. Дешево. Срочно. Касса у меня.';
$mat_text[]= 'Тошно от вас. '.$us.', выеби себя, моZет сблюю.';
$mat_text[]= 'Эй, '.$us.', хули мозги ебешь? Отвечай на вопрос нах!';
$mat_text[]= 'Слышь, '.$us.', я вот всё спросить хочу. Как те вообще Zивется с таким припизднутым ником? '.$us.' ... Пиздец.';
$mat_text[]= 'Всех местных уёбков ненавиZу, но '.$us.'-это просто пиздец!';
$mat_text[]= 'Внимание! '.$us.' объявляет конкурс на лучшего ебаря для своей задницы. Заявки принимаются в записки на ник '.$us.'';
...
if(!empty($text)){
$rand=rand(1,mysql_result(mysql_query("select count(id) from `bot_dialog`;"),0));
list($bot_text)=mysql_fetch_array(mysql_query("select message from `bot_dialog` where id='".$rand."';"));
}
else $bot_text='';
}
