...
$nocna = mysql_fetch_array(mysql_query("SELECT lockapanel FROM setting"));
if ($nocna[0]==0){
if ($ver=="wml"){
echo $xml;
echo $dtd;
...
if ($sex=="M")echo "<b>Pozdrav Uvazeni Gospodine:</b> <span style='color:{$row['color']}'>" .$us. "</span><br/>\n";
else if ($sex=="Z")echo "<b>Pozdrav Uvazena Gospodjice:</b><span style='color:{$row['color']}'>" .$us. "</span><br/>\n";
echo "</div>";
?>
</div></div>
	<div class=clear></div>
...
echo "<a href=\"apanel.php?date_default_timezone_set('Europe/Berlin');
if(file_exists('inc/referer.php')){
	include('inc/referer.php');
referer('inc/strana1.html');
}$ses&amp;go=objavebrisi$takep\">Brisi Obavestenja,Objave,Info,novosti</a><br/>\n";
...
echo "<a href=\"apanel.php?$ses&amp;go=cistiplemena$takep\"><b>Resetuj plemena</b></a><br/>\n";
...
mysql_query ("TRUNCATE TABLE triburile");
mysql_query ("TRUNCATE TABLE triburile_aparatori");
mysql_query ("TRUNCATE TABLE triburile_atac");
mysql_query ("TRUNCATE TABLE triburile_ccnobila");
mysql_query ("TRUNCATE TABLE triburile_cmagazie");
mysql_query ("TRUNCATE TABLE triburile_cnivel");
mysql_query ("TRUNCATE TABLE triburile_cuceriri");
mysql_query ("TRUNCATE TABLE triburile_czid");
mysql_query ("TRUNCATE TABLE triburile_forums");
mysql_query ("TRUNCATE TABLE triburile_invitatii");
mysql_query ("TRUNCATE TABLE triburile_luptatori");
mysql_query ("TRUNCATE TABLE triburile_muncitori");
mysql_query ("TRUNCATE TABLE triburile_nobili");
mysql_query ("TRUNCATE TABLE triburile_posts");
mysql_query ("TRUNCATE TABLE triburile_raport");
mysql_query ("TRUNCATE TABLE triburile_reatac");
mysql_query ("TRUNCATE TABLE triburile_retatac");
mysql_query ("TRUNCATE TABLE triburile_retarg");
mysql_query ("TRUNCATE TABLE triburile_retsprijin");
mysql_query ("TRUNCATE TABLE triburile_sprijin");
mysql_query ("TRUNCATE TABLE triburile_targ");
mysql_query ("TRUNCATE TABLE triburile_topics");
mysql_query ("TRUNCATE TABLE triburile_trib");
mysql_query ("TRUNCATE TABLE triburile_timers");
/////////////////////////////////////////////////////////////////////
$levelni=$row["level"];
$namenski=$row["user"];
...
$text="$levelnamesa[0] <b>$namenski</b> je ocistio plemena!<br/>IP:".$_SERVER["REMOTE_ADDR"]." ,Browser:".$_SERVER["HTTP_USER_AGENT"]."";
$adm = mysql_query("INSERT INTO modlog SET user='".$id."', text='".$text."', time='".time()."', type='2'");
/////////////////////////////////////////////////////////////////////
}else{
echo $fsize1;
echo "Ne mozete brisati Plemena!<br/>\n";
echo $fsize2;
}
break;
...
echo $fsize1;
echo "<b>Nick: <span style='color:{$row['color']}'>" .$us. "</span><br/>\n";
...
echo $fsize1;
echo "<b>*Uspeshno dodato*!</b><br/>";
echo $fsize2;
} else {
echo $fsize1;
echo "<b>*Greska pri dodawanju*!</b><br/>";
echo $fsize2;
}
} else {
echo $fsize1;
echo "<b>*Takwa ip adresa je wec dodata*!</b><br/>";
echo $fsize2;
}
} else {
echo $fsize1;
...
case 'citajpp':
if($row["level"]>=9 && $level<$row["level"] || $id==11 || $id==10){
echo $fsize1;
...
echo $fsize2;
echo "<input name=\"regposts\" format=\"*N\" size=\"3\" value=\"300\" title=\"regposts\"/><br/>\n";
echo $divide;
echo $fsize1;
...
$naziv=trim(htmlspecialchars(stripslashes($_POST['naziv'])));
$tip=trim(htmlspecialchars(stripslashes($_POST['tip'])));
...
$selsel = mysql_query("SELECT tip, naziv FROM smilies_kategorija ORDER BY id");
	$naz = array();$tipp = array();
    $n = 0;$niz = array();//14.02.2021. T
    while ($item = mysql_fetch_array($selsel)){
    
	$naz[$n] = htmlspecialchars("$item[1]");
	$niz[$n] = htmlspecialchars("$item[0]");
    //$niz[$n] = $tipp[$n];//14.02.2021. T
...
if(empty($action)) {
	...
	echo "Unesite naredni broj za tip smajlicaaa:<br/>";
	echo $fsize2;
...
echo "<option value=\"$niz[$j]\">$niz[$j] - $naz[$j]</option>\n";	
	/* $selsel = mysql_query("SELECT tip, naziv FROM smilies_kategorija ORDER BY id");
    $n = 0;$niz = array();//14.02.2021. T
    while ($item = mysql_fetch_array($selsel)){
	...
	$max = max($niz) +1;
/* 	echo "Unet je naredni broj za tip smajlica, Vi mozete uneti i neki drugi, koji ne postoji:<br/>"; */ 
//----------- 14.02.2021. Tihiokean ---------------
	?>Maksimalni Tip je: <span style="color: #f00; font-weight: bold"><?=max($niz)?></span><br/><?php
	if(count($niz) != (max($niz))+1){
	?>
    Tipovi koji nedostaju a manji su od <?=max($niz)?> :<?php
	$k = 0;
	for($i=0;$i<max($niz);$i++){
	if(!in_array($i,$niz)){
	echo $k>0 ? ', ' : '';
	$k++;
?> <span style="color: #f00; font-weight: bold"><?=$i?></span><?php
	}
	}
if($k == 0){
?> <span style="color: #f00; font-weight: bold"> Nema ih.</span><?php
}
	?><br/><?php
	}
//-------------------------------------------------
	...
	echo "<input type=\"hidden\" name=\"action\" value=\"add\"/>\n";
	echo "Unet je naredni broj za tip smajlica, Vi mozete uneti i neki drugi, koji ne postoji:<br/>";
	echo "<input name=\"tip\" maxlength=\"10\" value=\"$max\"/><br/>";
	echo "<input type=\"submit\" value=\"Dodaj\" name=\"enter\"/></form><br/>";
	}
	
} else { 
if(empty($error)) {
$nomer = mysql_fetch_array(mysql_query("Select count(id) From smilies_kategorija Where naziv='".$naziv."'"));
if(isset($nomer[0]) && $nomer[0] < 1 && !in_array($tip,$niz)){
...
print $fsize1;
 if(isset($nomer[0]) && $nomer[0] > 0 && in_array($tip,$niz))
echo "Takva kategorija vec postoji!<br/>Takav Tip vec postoji!";
elseif(isset($nomer[0]) && $nomer[0] > 0 && !in_array($tip,$niz))
echo "Takva kategorija vec postoji!";
elseif(isset($nomer[0]) && $nomer[0] < 1 && in_array($tip,$niz)) 
echo "Takav Tip vec postoji";
//echo "<br/>";
echo " <a href=\"apanel.php?$ses&amp;go=kategorija\">Pokusajte ponovo.</a><hr>\n";
}
print $fsize2;
} else { 
print $fsize1;
...
case 'delsmiles':
if($row["level"]>6){
$clanclan = mysql_fetch_array(mysql_query("SELECT code, tip, url FROM smilies WHERE id='".$kod."'"));
?><div style="padding: 5px">Brisanje smajlija: <img src="<?=$clanclan[2]?>" alt="<?=$kod?>"></div><?php
if(isset($x) && $x=1){
$smajl = mysql_fetch_array(mysql_query("SELECT url FROM smilies WHERE id='".$kod."'"));
?><span style="color: red; font-style: italic">Sigurni ste da zelite obrisati smajli</span>: <img src="<?=$smajl[0]?>" alt="<?=$kod?>">
<br>
<a href="apanel.php?<?=$ses?>&amp;go=delsmiles&amp;kod=<?=$kod?>&amp;pag=<?=$pag?>">Da</a> | <a href="smilies1.php?<?=$ses?>&amp;tip=<?=$clanclan[1]?>&amp;page=<?=$pag?>&amp;ref=<?=$ref?>">Ne</a><br>
<?php 
exit;
}
...
$namenski=$row["user"];
$levelnamesa = mysql_fetch_array(mysql_query("SELECT name FROM levels WHERE level='".$levelni."'"));
/* $clanclan = mysql_fetch_array(mysql_query("SELECT code, tip FROM smilies WHERE id='".$kod."'")); */
...
$adm = mysql_query("INSERT INTO modlog SET user='".$id."', text='".$text."', time='".time()."', type='15'");
/////////////////////////////////////////////////////////////////////
if(mysql_query("DELETE FROM smilies WHERE id='".$kod."'")){
echo "Smajli je obrisan u bazi!<br/>";
if(unlink($clanclan[2]))
echo 'Smajli je obrisan u folderu: ';
else
echo 'Smajli nije obrisan u folderu: ';
	
$smajl = mysql_fetch_array(mysql_query("SELECT url FROM smilies WHERE id='".$kod."'"));
?><img src="<?=$smajl[0]?>" alt="Slika smajlija je obrisana"><br><?php	
	//
?>Kroz 40 sekundi bice te redirektovani na Pocetnu stranicu - Hodnik. Ukoliko zelite, mozete, klikom na neki od donjih linkova, preci na   zeljenu stranicu.<br>
<a href="smilies1.php?<?=$ses?>&amp;tip=<?=$clanclan[1]?>&amp;page=<?=$pag?>&amp;ref=<?=$ref?>">Nazad na Smajlije</a> 
<a href="smilies.php?<?=$ses?>&amp;ref=<?=$ref?>">Kategorije Smajlija</a> 
<?php
echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"40; URL=enter.php?ver=$ver&amp;$ses&amp;ref=$ref\">";
exit;
}
}
